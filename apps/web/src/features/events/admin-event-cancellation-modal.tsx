...
export function AdminEventCancellationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 px-5 backdrop-blur-sm">
      <section className="max-h-[88vh] w-full max-w-sm overflow-y-auto rounded-[28px] bg-surface p-5 shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold text-ink">Cancel event?</h2>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <div className="rounded-2xl bg-surface-muted p-3">
            ...
            <p className="mt-1 text-lg font-semibold text-ink">{impact.paidTickets}</p>
          </div>
          <div className="rounded-2xl bg-surface-muted p-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-ink-400">
              Reserved
        ...
        <label className="mt-4 grid gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-ink-400">
          Cancellation reason
          <textarea
            className="min-h-24 rounded-2xl border border-black/8 px-4 py-3 text-sm font-medium normal-case leading-6 tracking-normal text-ink outline-none focus:border-brand focus:ring-1 focus:ring-brand"
            value={reason}
            onChange={(inputEvent) =>
          ...
          />
          <span className="text-right text-[11px] font-medium normal-case tracking-normal text-ink-300">
