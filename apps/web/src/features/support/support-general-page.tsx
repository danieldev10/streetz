...
export function SupportGeneralPage() {
  return (
    <SupportShell>
      <section className="overflow-hidden rounded-[32px] bg-ink p-7 text-white md:p-10">
        <div className="max-w-2xl">
          <div className="inline-flex items-center gap-2 rounded-full bg-surface/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.14em]">
        ...
        {supportDestinations.map((destination) => {
          ...
          return (
            <Link
              ...
              className="group rounded-[24px] border border-black/[0.06] bg-surface p-5 transition hover:border-black/15 hover:shadow-[0_10px_30px_rgba(0,0,0,0.04)]"
              href={destination.href}
            >
              <div className="flex items-center justify-between">
                <Icon className="size-5" aria-hidden="true" />
                <ChevronRight
                  className="size-4 text-ink-300 transition-transform group-hover:translate-x-0.5"
                  aria-hidden="true"
                />
              </div>
              <h2 className="mt-4 font-semibold">{destination.title}</h2>
              <p className="mt-2 text-sm leading-6 text-ink-600">{destination.description}</p>
            </Link>
          );
        })}
      </section>
      ...
      <Link
        className="mt-6 block rounded-[24px] border border-danger-border bg-danger-tint p-5 transition hover:border-danger-border"
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-3">
              <h2 className="font-semibold text-danger">Safety concern?</h2>
