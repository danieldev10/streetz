...
export function DiscoveryCandidateCard({
  ...
  return (
    <>
      <div className="relative h-[clamp(320px,44svh,440px)] bg-brand-tint md:aspect-[4/5] md:h-auto md:min-h-[440px]">
        ...
        <div
          className={`absolute left-5 top-5 rounded-[14px] border-2 px-4 py-2 text-lg font-semibold uppercase tracking-[0.08em] transition-opacity ${
            swipeIntent === "PASS"
              ? "border-white bg-surface/90 text-ink opacity-100"
        ...
        <div
          className={`absolute right-5 top-5 rounded-[14px] border-2 px-4 py-2 text-lg font-semibold uppercase tracking-[0.08em] transition-opacity ${
            swipeIntent === "LIKE"
              ? "border-brand bg-brand-strong/90 text-white opacity-100"
              : "border-brand/60 text-brand opacity-0"
          }`}
        >
          Like
        </div>
      ...
      <div className="p-4">
        {candidate.bio ? <p className="line-clamp-2 text-sm leading-6 text-ink-700">{candidate.bio}</p> : null}
        ...
        <button
          className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-full border border-black/[0.08] bg-surface px-4 text-sm font-medium text-ink disabled:cursor-not-allowed disabled:opacity-60"
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <button
            className="inline-flex h-11 items-center justify-center rounded-full border border-black/[0.08] text-ink disabled:cursor-not-allowed disabled:opacity-60"
            onClick={onPass}
            onPointerDown={(event) => event.stopPropagation()}
            disabled={isActionDisabled}
            aria-label="Pass"
          ...
          <button
            className="inline-flex h-11 items-center justify-center rounded-full bg-brand-strong text-white disabled:cursor-not-allowed disabled:opacity-60"
            onClick={onLike}
            onPointerDown={(event) => event.stopPropagation()}
            disabled={isActionDisabled}
            aria-label="Like"
        ...
        <div className="mt-2 grid grid-cols-2 gap-3">
          <button
            className="inline-flex h-9 items-center justify-center gap-2 rounded-full bg-surface-muted px-3 text-xs font-medium text-ink-600 disabled:cursor-not-allowed disabled:opacity-60"
            onClick={onBlock}
            onPointerDown={(event) => event.stopPropagation()}
            disabled={isActionDisabled}
          >
